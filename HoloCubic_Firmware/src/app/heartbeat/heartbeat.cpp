...
static int heartbeat_init(AppController *sys)
{
    ...
    if (size != 0) // 如果已经设置过heartbeat了，则开启mqtt客户端
    {
        ...
        {
            hb_cfg.mqtt_client = new PubSubClient(hb_cfg.mqtt_server, 1883, hb_cfg.callback, hb_cfg.espClient);
        }
        // 连接wifi，并开启mqtt客户端
