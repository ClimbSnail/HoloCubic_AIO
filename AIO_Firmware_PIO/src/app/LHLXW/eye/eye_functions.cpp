...
//改成240*240大小后，数据从pbuffer转移到pbuffer_m花费大量时间
//改成240*240大小后，BUFFER_SIZE必须为128，确保一次渲染一整行，屏幕实际更新两行（也就是一次渲染128个像素，根据这128个像素同时一次更新240*2个像素）
extern uint16_t *pbuffer; // 眼珠子算法渲染的缓冲区
extern uint16_t *pbuffer_m;//实际刷新到屏幕的缓存



extern eye_run *e_run;
extern bool eye_loop(void);
...
#endif
    e_run->startTime = millis(); // For frame-rate calculation
}
  ...
  for (screenY = 0; screenY < SCREEN_HEIGHT; screenY++, scleraY++, irisY++) {
    ...
    for (screenX = 0; screenX < SCREEN_WIDTH; screenX++, scleraX++, irisX++, lidX += dlidX) {
      if ((pgm_read_byte(lower + screenY * SCREEN_WIDTH + lidX) <= lT) ||
        ...
        #if EYE_MIN
        if (d < IRIS0_MAP_HEIGHT) {                      // Within iris area
            a = (IRIS0_MAP_WIDTH * (p >> 7)) / 512;        // Angle (X)
            p = pgm_read_word(iris0 + d * IRIS0_MAP_WIDTH + a);                           // Pixel = iris
        } else {                                        // Not in iris
        ...
        }else if( e_run->mode_eye < 6){
            if (d < IRIS1_MAP_HEIGHT) {                      // Within iris area
                a = (IRIS1_MAP_WIDTH * (p >> 7)) / 512;        // Angle (X)
                p = pgm_read_word(iris1 + d * IRIS1_MAP_WIDTH + a);                           // Pixel = iris
            } else {                                        // Not in iris
                p = pgm_read_word(sclera + scleraY * SCLERA_WIDTH + scleraX);               // Pixel = sclera
            }
        }else if( e_run->mode_eye < 9){
            if (d < IRIS2_MAP_HEIGHT) {                      // Within iris area
                a = (IRIS2_MAP_WIDTH * (p >> 7)) / 512;        // Angle (X)
                p = pgm_read_word(iris2 + d * IRIS2_MAP_WIDTH + a);                           // Pixel = iris
            } else {                                        // Not in iris
                p = pgm_read_word(sclera + scleraY * SCLERA_WIDTH + scleraX);               // Pixel = sclera
            }
        }else{
            if (d < IRIS3_MAP_HEIGHT) {                      // Within iris area
                a = (IRIS3_MAP_WIDTH * (p >> 7)) / 512;        // Angle (X)
                p = pgm_read_word(iris3 + d * IRIS3_MAP_WIDTH + a);                           // Pixel = iris
            } else {                                        // Not in iris
                p = pgm_read_word(sclera + scleraY * SCLERA_WIDTH + scleraX);               // Pixel = sclera
            }
        }
      ...
      *(&pbuffer[0] + pixels++) = p >> 8 | p << 8;//更改标记
    }
    if( e_run->mode_eye == 2|| e_run->mode_eye == 5|| e_run->mode_eye == 8|| e_run->mode_eye == 11){/* 双大眼 */
        for(uint8_t i=0;i<120;i++){//x从128均匀扩充到240
            pbuffer_m[i] = pbuffer[i+4];
            pbuffer_m[i+120] = pbuffer[i+4];
        }
        if(screenY>=4&&screenY<124){
            tft->pushPixels(pbuffer_m, 240);//y从1扩充到2(y一次只扩充2)
            tft->pushPixels(pbuffer_m, 240);  
        } 
    }else if( e_run->mode_eye == 0|| e_run->mode_eye == 3|| e_run->mode_eye == 6|| e_run->mode_eye == 9){/* 小眼 */
        ...
        for(uint8_t i=0;i<120;i++){//x从128均匀扩充到240
            if(i<56||i>=64){
                pbuffer_m[i*2] = pbuffer[temp__];
                pbuffer_m[i*2+1] = pbuffer[temp__];
                temp__ += 1;
            }else {
                pbuffer_m[i*2] = pbuffer[temp__];
                temp__ += 1;    
...
bool frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  if (!(++frames & 255)) { // Every 256 frames...
    float elapsed = (millis() -  e_run->startTime) / 1000.0;
...
void split( // Subdivides motion path into two sub-paths w/randimization
  ...
  if (range >= 8) {    // Limit subdvision count, because recursion
    ...
    while ((dt = (micros() - startTime)) < duration) {
      ...
      if(frame(v) == false)        // Draw frame w/interim iris scale value
        return;//用户发出了退出指令
    }
  }
}
