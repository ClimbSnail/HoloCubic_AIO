...
//扩展组件属性更新函数
static void extension_1_update(struct Remote_Sensor info);
static void extension_2_update(struct Remote_Sensor info);
static void extension_3_update(struct Remote_Sensor info);
static void extension_4_update(struct Remote_Sensor info);
static void extension_5_update(struct Remote_Sensor info);
 ...
 * @brief UI样式显示初始化
 */
void display_remote_sensor_gui_init(void)
{
 ...
 * @brief UI界面显示初始化
 */
void display_remote_sensor_init(void)
{
 ...
 */
void display_remote_sensor(struct Remote_Sensor sensorInfo)
{
    display_remote_sensor_init();
 ...
 * @brief UI界面对象释放
 */
void remote_sensor_gui_release(void)
{
    if (scr != NULL)
    {
        lv_obj_clean(scr);
