...
#define REMOTE_SENSOR_APP_NAME "Remote Sensor"

//数据解析表 - 前导字符串
static const char *rs_data_header[] = {
    "CPU usage", "CPU temp", "CPU freq", "CPU power", "GPU usage",
    "GPU temp", "GPU power", "RAM usage", "RAM use", "NET upload speed",
    "NET download speed",
};
//数据解析表 - 单位
...
//传感器组件的持久化配置
#define REMOTE_SENSOR_CONFIG_PATH "/remote_sensor.cfg"
struct RS_Config
{
...
// APP运行数据
static RemoteSensorAppRunData *run_data = NULL;

//配置信息
static RS_Config cfg_data;

static void write_config(RS_Config *cfg)
{
    char tmp[16];
    // 将配置数据保存在文件中（持久化）
    String w_data;
    ...
    w_data += tmp;
    g_flashCfg.writeFile(REMOTE_SENSOR_CONFIG_PATH, w_data.c_str());
}

static void read_config(RS_Config *cfg)
{
    // 如果有需要持久化配置文件 可以调用此函数将数据存在flash中
    // 配置文件名最好以APP名为开头 以".cfg"结尾，以免多个APP读取混乱
    char info[64] = {0};
    uint16_t size = g_flashCfg.readFile(REMOTE_SENSOR_CONFIG_PATH, (uint8_t *)info);
    info[size] = 0;
    if (size == 0)
    {
        // 默认值
 ...
 * @brief 遥感器数据解析
 */
static void remote_sensor_data_del(String line)
{
    int16_t dataStart = 0;
    int16_t dataEnd = 0;
    String dataStr;
 ...
 * @brief 获取遥感器数据
 */
static void get_remote_sensor_data(void)
{
    ...
    //尝试通过IP地址连接主机
    if (!run_data->client->connect(run_data->host, 80))
    {
    ...
    //解析数据
    remote_sensor_data_del(line);
}

/**
 * @brief app初始化
 */
static int remote_sensor_init(AppController *sys)
{
    tft->setSwapBytes(true);
    display_remote_sensor_gui_init();
    // 获取配置信息
    read_config(&cfg_data);

    // 初始化运行时参数
    run_data = (RemoteSensorAppRunData *)calloc(1, sizeof(RemoteSensorAppRunData));
    run_data->preSensorMillis = 0;
    run_data->preTimeMillis = 0;
    memset(&run_data->rs_data, 0, sizeof(Remote_Sensor));
 ...
 * @brief app进程
 */
static void remote_sensor_process(AppController *sys, const ImuAction *act_info)
{
    ...
    {
        //发送更新数据显示事件
        sys->send_to(REMOTE_SENSOR_APP_NAME, CTRL_NAME,
 ...
 * @brief app后台任务
 */
static void remote_sensor_background_task(AppController *sys, const ImuAction *act_info)
{
    // null
}
 ...
 * @brief app退出回调函数
 */
static int remote_sensor_exit_callback(void *param)
{
    remote_sensor_gui_release();
 ...
 * @brief app消息处理
 */
static void remote_sensor_message_handle(const char *from, const char *to,
                                         ...
                                         void *ext_info)
{
    ...
    {
        Serial.println(F("----->remote_sensor_event_notification"));
        int event_id = (int)message;
        switch (event_id)
        {
        case UPDATE_RS_DATA:
        {
            Serial.print(F("remote_sensor update data.\n"));

            get_remote_sensor_data();
            display_remote_sensor(run_data->rs_data);
        };
        break;
        default:
            break;
...
APP_OBJ remote_sensor_app = {REMOTE_SENSOR_APP_NAME, &app_remote_sensor, "",
                             remote_sensor_init, remote_sensor_process, remote_sensor_background_task,
                             remote_sensor_exit_callback, remote_sensor_message_handle};
